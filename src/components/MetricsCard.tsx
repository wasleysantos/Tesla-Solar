...
export function MetricsCard({ icon, label, value, color, valueColor }: MetricsCardProps) {
  ...
  return (
    <div className={`bg-[#1a2942] rounded-2xl p-4 border-b-4 ${currentTextBorder.split(' ')[1]}`}>
      <div className={`${currentBg} ${currentTextBorder.split(' ')[0]} w-10 h-10 rounded-lg flex items-center justify-center mb-3`}>
        {icon}
      </div>
